...
eval_camflow_apt_cpu_mem:
	...
	cd data/camflow-apt/test && mkdir -p base && mkdir -p stream
	make -C build/parsers/camflow eval_camflow_apt_prepare
	make -C build/parsers/camflow eval_camflow_apt WINDOW=3000 INTERVAL=10000
	make -C build/graphchi-cpp eval_camflow_apt_cpu_mem WINDOW=3000 INTERVAL=10000
...
run_spade_apt_subset:
	...
	cd data/spade-wget-apt/train && mkdir -p base && mkdir -p stream
	cd data/spade-wget-apt/test && mkdir -p base && mkdir -p stream
	cd build/parsers/cdm && make spade_apt_subset
	cd build/graphchi-cpp && make spade_apt_subset
...
spade_apt_subset_hotfix_CV: prepare_parsers prepare_graphchi_hotfix prepare_modeling prepare_output download_spade_apt run_spade_apt_subset
...
run_camflow_shellshock_subset:
	...
	cd data/shellshock-apt/train && mkdir -p base && mkdir -p stream
	cd data/shellshock-apt/test && mkdir -p base && mkdir -p stream
	cd build/parsers/cdm && make camflow_shellshock_subset
	cd build/graphchi-cpp && make camflow_shellshock_subset
	cd build/modeling && python model.py --train_dir ../../data/shellshock-apt/train_sketch/ --test_dir ../../data/shellshock-apt/test_sketch/ > results.txt

camflow_shellshock_subset_hotfix_CV: prepare_parsers prepare_graphchi_hotfix prepare_modeling prepare_output download_camflow_shellshock run_camflow_shellshock_subset
